...
============================================================
TAKEOVER INTENT — READ BEFORE ANY M4 (Trace≠Human-Driving) JUDGMENT
============================================================
The caption must match the CORRECT post-disengagement behavior, which is what the HUMAN driver did after taking over (human = ground truth) — judged by the human's CORRECTIVE INTENT, not by raw speed numbers. First infer WHY the human disengaged, using the triage note + the disengagement moment:
- If the AUTO car acted UNSAFELY EARLY (e.g. began moving / crept forward while a pedestrian, crossing guard, or hazard was still present) and the human took over to STOP or HOLD, then the correct behavior is to REMAIN STOPPED / wait until clear. A trace/plan that says "remain stopped until clear" is CORRECT here — do NOT treat it as a contradiction or imply it should "proceed," even if the vehicle eventually moves later.
- If the AUTO car was OVER-CAUTIOUS (sat when already safe) and the human took over to GO, then the correct behavior is to PROCEED, and a plan that says "keep waiting" is the problem.
Triage phrases like "poor yield / underyield / moved too early" describe the ADV's FAILURE that triggered takeover — they characterize what the AUTO car did wrong, NOT a claim that the human drove badly. Never phrase a fix as the human's driving being "poor" or "bad"; describe the correct maneuver neutrally. Since M4 is scene-dependent it must still be surfaced as a CAMERA CHECK (never flagged from text), but do NOT invent a contradiction between a cautious "wait until clear" plan and the footage just because a stop was involved.
...
F. LABEL GUIDANCE — Advise conservatively on Minimal Input. Default SVC-F. Recommend an additional camera ONLY if the correct driving DECISION genuinely requires it. BE DECISIVE on Temporal: if the scene matches a canonical Temporal case (active school bus with flashing lights/stop sign, vehicle cut-in, pedestrian/animal motion, traffic-light state change), say "yes" — not "maybe". Use "maybe" only when genuinely ambiguous, "no" for static object / single-frame signal. Note any selection inconsistency (m16/m17). Frame as a hint to confirm.
   MISSED TEMPORAL (text-only): even without frames, if the TEXT itself describes decision-relevant MOTION — a vehicle "cutting in / merging / changing lanes", a pedestrian/cyclist/animal "crossing / walking / approaching / moving", a light "turning green/red", a lead car "beginning to move / slowing / stopping" — and the tasker did NOT select Temporal, flag it: set temporal_needed="yes" and note the described motion implies Temporal was required. Be honest about the limit: if the caption does NOT describe motion, you cannot know from text alone whether real motion occurred, so surface Temporal as a CAMERA-CHECK question ("Did any vehicle/pedestrian/light MOVE across frames? If so, Temporal is needed") rather than assuming it was fine.
...
ACTIVE MOTION CHECK (for Temporal): explicitly COMPARE the frames in time order. Ask: does anything MOVE across the frames in a way that is decision-relevant — a vehicle cutting in or changing lanes, a pedestrian/cyclist/animal crossing or approaching, a traffic light changing state, a lead vehicle starting or stopping? If YES and the tasker did NOT select Temporal, set temporal_needed="yes" and say plainly that motion is visible across frames that requires Temporal (this is a real m17/minimal-input miss, not just a hint). If the frames show only static scenes, Temporal is correctly not needed. Motion you can SEE across frames overrides the text: even if the caption never mentions the movement, if the frames show decision-relevant motion, flag that Temporal was needed.
...
${textRoute ? "This Text Route is authoritative context provided with the task (the intended route/maneuver). The tasker did NOT author it and must NOT edit it. Use it ONLY to check consistency, and ACTIVELY CROSS-CHECK the final plan's maneuver against the Text Route's next instruction: if the Thinking Trace or Driving Plan describes a maneuver or direction that CONTRADICTS the Text Route (e.g. the Text Route says 'turn left in 50 m' but the plan continues straight; Text Route says a left turn but the plan executes a right turn; Text Route says a U-turn but the plan keeps the lane), flag it as an M3 consistency problem and frame the fix as making the trace/plan consistent with the given Text Route (then confirm the true situation on camera). Do NOT flag the Text Route itself as an error, do NOT suggest editing it, and if it is silent on something do not treat that as a contradiction." : ""}
