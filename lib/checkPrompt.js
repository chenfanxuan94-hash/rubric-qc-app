...
import {
  ...
  FLAG_DISCIPLINE, CAMERA_LIST, CONTRADICTION_REASONING, CAMERA_CHECK_GUIDANCE, STRUCTURE_GUIDANCE,
...
NEVER flag these as contradictions:
...
These are camera_checks ONLY — never a major/minor flag, never a highlighted span. Keep the flags list strictly text-demonstrable so it stays uncluttered.
...
GRAMMAR SPANS (m15): each grammar/typo finding is its OWN point with a TIGHT span covering only the broken words (e.g. just "the the"), never a long span across an unrelated clause, never two grammar problems merged.

m16 fires ONLY when a DECISION-CRITICAL object is referenced that would only be visible in a non-front camera. Scenery in a direction ("accident on the left") does NOT trigger m16. m17 = Minimal Input left blank.

m13 (missing plan option) is a GENTLE NOTE, not a scored flag — list it under minor_flags only if NO alternative is weighed at all, and keep its fix soft; do not punish a thin-but-correct trace.

============================================================
TAG NAMES
============================================================
Every finding's "type" must be the SHORT REAL RUBRIC NAME, never an invented word. Use: M3→"Trace↔Plan mismatch", M4→"Trace≠human driving", M5→"Trajectory", M6→"Speed", M7→"Safety/Compliance", M8→"High-prominence", m9/m10→"Hallucination", m11/m12→"Missing object", m13→"Missing plan option", m14→"Logical flow", m15→"Grammar & syntax", m16→"Camera mismatch", m17→"Minimal Input blank". For a contradiction, you may use "contradiction". Do NOT output words like "structure" or "consistency" as the type.
