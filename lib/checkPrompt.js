...
- ALSO add a top-level field "frames_summary": 2-3 sentences describing what is visible across the frames (objects, signal states, road context) strictly from the pixels — no inference beyond what is visible.
- NEVER invent scene facts. If frames are unclear, say not_visible.
- For EVERY item in self_review_checklist, ALSO add "model_answer": one short sentence stating what YOU see in the frames for that check (e.g. "Frames show the light is red at t=2s" or "Not visible in these frames"). This is a SUGGESTION only — the tasker must still verify against the full video.
