...
export function lintField(text, where) {
  ...
  HEDGE.forEach((w) => {
    findWord(text, w).forEach((h) => {
      issues.push({
        ...
        suggestion: "", // removing the hedge word is the deterministic fix
  ...
  Object.entries(ABBREV).forEach(([ab, exp]) => {
    ...
    findWord(text, ab).forEach((h) => {
      issues.push({
        ...
        suggestion: null, // expansion is contextual — let the tasker choose
  ...
  NON_FIRST.forEach((w) => {
    findWord(text, w).forEach((h) => {
      const isCap = h.text[0] === h.text[0].toUpperCase();
      issues.push({
        where, kind: "person", original: h.text, start: h.start, end: h.end,
        suggestion: isCap ? "I" : "I", // first person
