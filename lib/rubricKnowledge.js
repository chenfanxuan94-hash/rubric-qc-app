...
export const CONTRADICTION_REASONING = {
  traceIsTemporal: "A Thinking Trace moves through TIME in stages: (1) Analyze input = the ADV's CURRENT state and what it observes right now; (2) Synthesize/Reason = weighing options; (3) Explore Alternatives; (4) Final Plan = the COMMITTED future action. A statement in stage 1 about where the ADV currently is, or what an agent is currently doing, is an OBSERVATION at that moment. A statement in the plan about where the ADV will go is the DECISION. These describe different moments and almost never contradict each other.",
  whatIsAContradiction: "A real contradiction = two claims about the SAME attribute at the SAME moment that cannot both be true. Examples: 'the light is green' AND 'the light is red'; the committed ACTION is 'maintain 40 mph' AND also 'brake to a complete stop'; 'I am in the middle lane' AND 'I am in the right lane' stated as the SAME current position.",
  whatIsNOT: [
    "Current position vs target/plan position is NOT a contradiction. 'I am currently in the middle lane' (observation) + 'I will move to / stay in the right-most lane' (plan) = normal driving. DO NOT flag.",
    "An observation of another agent ('a blue sedan is moving ahead and to the right') is a description, NOT a contradiction by itself. DO NOT flag it as M5.",
    "Describing the current speed then deciding to change it ('I am at 40 mph' + 'I will decelerate') is NOT a contradiction.",
    "Mentioning an object's location ('accident on the left') is description, not conflict.",
  ],
  phantomLeadRule: "The 'phantom lead vehicle' issue may ONLY be flagged when the TEXT CONTRADICTS ITSELF about the same vehicle — e.g. it calls the car a same-lane 'lead vehicle I am following' in one place AND places it in an 'adjacent lane' in another. If the text simply describes a car in an adjacent lane, or simply says it is following a lead car, with no internal conflict, DO NOT flag it. If you are merely unsure whether the car is really in-lane, that is a CAMERA CHECK, not a flag.",
};
