...
export const CONTRADICTION_REASONING = {
  traceIsTemporal: "A Thinking Trace moves through stages: (1) Analyze input = the ADV's CURRENT state and observations; (2) Synthesize/Reason and (3) Explore Alternatives = candidate plans, INCLUDING ones it deliberately rejects; (4) Final Plan = the COMMITTED action. Explored/rejected options are SUPPOSED to differ from the final plan — that is the reasoning working correctly.",

  // The ONLY things that count as a contradiction/error:
  triggers: [
    "DECISION FLIP: the trace COMMITS to something ('I have decided to stop', 'I will yield') and then the FINAL plan does something genuinely different ('I will proceed through'). Two COMMITTED statements that cannot both be true.",
    "SAME-OBJECT ATTRIBUTE MISMATCH: the same object is described with conflicting attributes between the observation and the plan/action — e.g. input says a BLACK car, final action says avoiding the SILVER car; input says the bus is in the RIGHT lane, plan treats it as in MY lane. Same object, incompatible attribute.",
    "SAFETY-CRITICAL OMISSION (Major, M7/M4): the observation notes a safety-critical element — police officer directing traffic, ACTIVE school bus (stop sign out / lights flashing), red light, pedestrian in path, stop sign — but the FINAL plan / driving plan does NOT carry it into the rationale or the committed action. Either it is entirely absent from the plan's reasoning, OR the committed action does not respond to it (e.g. observes active school bus but plans to proceed). Both are Major.",
  ],

  // Things that must NEVER be flagged as contradictions:
  neverFlag: [
    "An explored or REJECTED alternative differing from the final plan ('Option: change lanes (rejected)' … 'I will stay in lane'). This is correct reasoning, not a contradiction.",
    "Paraphrases or differences of DEGREE that share the same intent: 'brake firmly to a complete stop' vs 'a hard brake is unnecessary' = the same controlled-stop intent. NOT a contradiction.",
    "Current state vs planned state: 'I am in the middle lane' (now) + 'I will stay in the right-most lane' (plan) = normal driving. NOT a contradiction.",
    "Describing current speed then deciding to change it ('I am at 40 mph' + 'I will decelerate'). NOT a contradiction.",
    "Mentioning an object's location as scenery ('accident on the left'). NOT a contradiction.",
  ],

  phantomLeadRule: "The 'phantom lead vehicle' issue may ONLY be flagged when the TEXT CONTRADICTS ITSELF about the same vehicle — calls it a same-lane 'lead vehicle I am following' in one place AND places it in an 'adjacent lane' in another. A plain description of a car in an adjacent lane, or simply following a lead car, is NOT flagged. If you are merely unsure whether the car is in-lane, that is a CAMERA CHECK, not a flag.",

  workedExamples: [
    "Trace: 'I am in the middle lane.' Plan: 'I will stay in the right-most lane.' → NOT a contradiction (current vs plan). At most a camera check 'Lane: middle or right?'.",
    "Speed: 'decelerate, possibly braking firmly to a full stop' and 'a hard brake is unnecessary as there are no hazards.' → NOT a contradiction (same controlled-stop intent, difference of degree). Do not flag.",
    "Input: 'a black sedan ahead.' Final action: 'I will nudge around the silver sedan.' → CONTRADICTION (same object, color mismatch). Flag.",
    "Observation: 'school bus stopped with red lights flashing and stop sign extended.' Final plan: 'I will continue straight at 25 mph.' → SAFETY-CRITICAL OMISSION (Major) — the bus is not addressed in the action.",
  ],
};
