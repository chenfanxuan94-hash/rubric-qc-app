...
// ---------------------------------------------------------------------------
// MINIMAL INPUT / TEMPORAL ground truth from the 5 examples.
// Base camera is ALWAYS SVC-F. Temporal is added only when the decision
// depends on MOTION across frames.
// ---------------------------------------------------------------------------
export const MINIMAL_INPUT_GUIDANCE = {
  base: "SVC-F is always the base camera.",
  temporalYes: "Add Temporal when the decision depends on motion/change across frames — e.g. a person GESTURING (officer waving you on), or a school bus's lights FLASHING / determining a bus is 'active'.",
  temporalNo: "Do NOT add Temporal for a static object in the lane (FOD), an observed signal state (a yellow light you can read in one frame), or a static road-surface hazard (potholes).",
  examples: [
    "FOD object in lane → SVC-F only (static object).",
    "Officer gesturing to proceed → SVC-F + Temporal (gesture is motion).",
    "Yellow light, stop at line → SVC-F only (light state readable in one frame).",
    "Active school bus → SVC-F + Temporal (flashing lights / active state).",
    "Potholes / degraded road → SVC-F only (static hazard).",
  ],
};
