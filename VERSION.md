...
3. **One-line verdict** in large plain text.
4. **+ Why / full explanation** — the detailed reasoning (Trace↔Plan consistency is folded in here; the old duplicate consistency box is gone).
5. **+ Issues to resolve (N)** — each issue has **✓ Addressed** and **✕ Disagree**. Acting on one fades it out and removes it from the list AND from the highlighted text below (they're linked). **Disagree** suppresses that issue on future re-checks (your call is respected); **Addressed** re-evaluates on next check. If you mark Addressed but the text didn't change, it asks "did you fix it, or disagree?". Every action is logged.
6. **Camera checks** — now a required gate: each must be marked **Checked** or **Not relevant** (it disappears). You cannot Submit until all are cleared. (Dropped the "AI can't see" line.)
7. **Label guidance** — the recommendation (e.g. SVC-F + Temporal) shown big; explanation behind a "+".
8. **Collapsed by default:** highlighted revised text, grammar, SOP standards, pre-seed→revised diff, full rubric sweep.
- TYPOGRAPHY: darker/near-black text, larger sizes, fewer words throughout.
- FEEDBACK LOOP: Agree/Disagree/Addressed signals are saved to a new `feedback` table. Reviewer can **Download feedback** (CSV) from the dashboard and hand it back to tune prompts/examples over time. (Builds a labeled dataset; does not retrain the live model automatically.)

### DB migration for v3.5.0 (run once in Supabase SQL editor)
    create table if not exists public.feedback (
      id bigint generated by default as identity primary key,
      created_at timestamptz not null default now(),
      tasker_name text, task_id text, code text, where_field text, severity text,
      issue_fix text, issue_why text, evidence text, action text,
      text_changed boolean, note text
    );
    alter table public.feedback enable row level security;

## v3.4.1
- Pinned ALL dependencies to EXACT versions (removed `^` ranges) so nothing auto-drifts to a possibly-compromised release. (Caught supabase-js silently drifting 2.45→2.107 via caret.)
- Upgraded Next.js 14.2.15 → 15.5.19 to clear all known security advisories (SSRF, cache-poisoning, DoS, image-optimizer, XSS); forced patched postcss 8.5.15 via `overrides`. `npm audit` is now CLEAN: 0 vulnerabilities. App still builds and runs on the Pages Router.
- Committed package-lock.json (sha512 integrity hashes for every package). Use `npm ci` for installs.
- Hardened .gitignore to cover all `.env*` variants while keeping the safe `.env.local.example` template.
- Added SECURITY.md (key rotation, supply-chain, npm ci guidance). No real secrets are in the repo.
